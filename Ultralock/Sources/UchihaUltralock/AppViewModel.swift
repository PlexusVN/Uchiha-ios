...
final class AppViewModel: NSObject, ObservableObject {
    ...
    func login() async {
        ...
        guard let encodedKey = key.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              ...
              let url = URL(string: "https://plexus-auth-api-o01h.onrender.com/api/verify?key=\(encodedKey)&hwid=\(encodedHw)") else {
            authState = .error("Invalid URL")
            return
        }

        var request = URLRequest(url: url)
        request.setValue(secret, forHTTPHeaderField: "X-Auth-Secret")
        ...
        do {
            ...
            if let serverHmac = json["hmac"] as? String {
                ...
                guard serverHmac == computed else {
                    authState = .error("Response integrity check failed")
                    return
                }
            }
    ...
    private func reVerify() async {
        ...
        guard let encodedKey = key.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let encodedHw = hw.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let url = URL(string: "https://plexus-auth-api-o01h.onrender.com/api/verify?key=\(encodedKey)&hwid=\(encodedHw)") else { return }
        ...
        do {
            ...
            if json["success"] as? Bool == true {
                ...
                if let serverHmac = json["hmac"] as? String {
                    ...
                    let expires = json["expires_at"] as? String ?? ""
                    let computed = HMACUtils.compute(key: secret, message: "\(success)|\(rawType)|\(expires)")
                    guard serverHmac == computed else { return }
                }
